...
export default function NewsDetailComponent({date, news_type, title, images, description}) {
  ...
  return (
    <section className={`${cls.news_detail} container`}>
      <div className={`${cls.news_detail__head} d-flex justify-content-between`}>
        <p className={`m-0`}>{formatDate(date)}</p>
        <p className={`m-0`}>{news_type}</p>
      </div>

      <h2 className={`sections__heading`}>{title}</h2>
        ...
        <Swiper navigation={true}>
          {
            data && data.map(({id, image}) => (
              <SwiperSlide key={id}>
                <div className={cls.news_detail__swiper_img}>
      ...
      <div className={cls.news_detail__description}>
        {description}
      </div>
    </section>
  )
